...
$pythonProbe = $null
try { $pythonProbe = & $python -c "import anthropic; print(anthropic.__file__)" 2>$null } catch { $pythonProbe = $null }
if ($LASTEXITCODE -ne 0 -or -not $pythonProbe) {
  $python = "C:\Program Files\PostgreSQL\18\pgAdmin 4\python\python.exe"
}
$pythonProbe = & $python -c "import anthropic; print(anthropic.__file__)" 2>$null
if ($LASTEXITCODE -ne 0 -or -not $pythonProbe) { throw "Pacote anthropic nao esta disponivel no runtime Python: $python" }
